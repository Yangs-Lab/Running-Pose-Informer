...
public class UpdateManager {
    ...
    public UpdateManager(JSONArray args, CallbackContext callbackContext, Context context, JSONObject options) {
        this(args, callbackContext, context,
             "https://raw.githubusercontent.com" +
             "/vsd-advantech/VEGA-Mobile-Store/master/version.xml", options);
    }
            ...
            switch (msg.what) {
              //case Constants.VERSION_COMPARE_START:
                case Constants.DOWNLOAD_START:
                    downloadHandler.sendEmptyMessage(Constants.DOWNLOAD_CLICK_START);
                    break;
              //case Constants.VERSION_THREAD_END:
                case Constants.DOWNLOAD_END:
                    callbackContext.success(Utils.makeJSONex2(Constants.DOWNLOAD_END, 
                ...
                case Constants.DOWNLOAD:
                    callbackContext.success(Utils.makeJSONex2(Constants.DOWNLOAD, 
                                            "downloading", downloadApkThread.progress));
                    break;
                case Constants.DOWNLOAD_FINISH:
                    isDownloading = false;
                    callbackContext.success(Utils.makeJSONex2(Constants.DOWNLOAD_FINISH, 
                                            "downloaded", 100));
                    downloadHandler.sendEmptyMessage(Constants.DOWNLOAD_END);
                    break;
                case Constants.DOWNLOAD_INSTALL_FINISH:
                    callbackContext.success(Utils.makeJSONex2(Constants.DOWNLOAD_INSTALL_FINISH, 
                ...
                case Constants.REMOTE_FILE_NOT_FOUND:
                    ...
                    break;
                default:
                    callbackContext.error(Utils.makeJSON(Constants.UNKNOWN_ERROR, "error, unknown"));
            }
        }
    };

    private void downloadApkEx() {
        LOG.d(TAG, "downloadApkEx");

        downloadApkThread = new DownloadApkThread(mContext, downloadHandler, 
                      null, null, downloadUpdateThread.getMHashMap(), options);
        this.cordova.getThreadPool().execute(downloadApkThread);
    }

    private CheckUpdateThread downloadUpdateThread;
        
    public void downloadUpdate() {
        LOG.d(TAG, "downloadUpdate..");

        downloadUpdateThread = new CheckUpdateThread(mContext, downloadHandler, queue, 
                                                    packageName, updateXmlUrl, options);
        this.cordova.getThreadPool().execute(downloadUpdateThread);
    }
    // -----------------------------------------------------
    
    // Byron added, 2020/02/26 -----------------------------
    private Handler infoHandler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            switch (msg.what) {
              //case Constants.VERSION_COMPARE_START:
                case Constants.VERSION_THREAD_START:
                    infoHandler.sendEmptyMessage(Constants.VERSION_THREAD_END);
                    break;
                case Constants.VERSION_THREAD_END:
                    Version version = queue.get(0);
                    int localCode = version.getLocal();
                    int remoteCode = version.getRemote();
                    String localName = version.getLocalName();
                    String remoteName = version.getRemoteName();
                    callbackContext.success(Utils.makeJSONex(Constants.VERSION_THREAD_END, 
                                "success", localCode, remoteCode, localName, remoteName));
                    break;
                case Constants.NETWORK_ERROR:
                    callbackContext.error(Utils.makeJSON(Constants.NETWORK_ERROR, "network error"));
                    break;
    ...
    private CheckUpdateThread getUpdateInfoThread;

    public void getUpdateInfo() {
        LOG.d(TAG, "getUpdateInfo..");

        getUpdateInfoThread = new CheckUpdateThread(mContext, infoHandler, queue, 
                                                    packageName, updateXmlUrl, options);
        this.cordova.getThreadPool().execute(getUpdateInfoThread);
    }
    // -----------------------------------------------------
