...
public class CheckAppUpdate extends CordovaPlugin {
    ...
    private boolean is_checkAppUpdate = true;
    // -----------------------------------------------------
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        // Byron added, 2020/02/26 -----------------------------
        if (action.equals("getAppUpdateInfo")) {
            is_checkAppUpdate = false;
            getUpdateManager().options(args, callbackContext);
            if (verifyOtherPermissions())
                getUpdateManager().getUpdateInfo();
            return true;
        }
        //
        if (action.equals("downloadAppUpdate")) {
            is_checkAppUpdate = false;
            getUpdateManager().options(args, callbackContext);
            if (verifyInstallPermission() && verifyOtherPermissions())
                getUpdateManager().downloadUpdate();
            return true;
        }
        // -----------------------------------------------------
