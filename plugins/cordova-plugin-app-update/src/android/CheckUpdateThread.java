...
public class CheckUpdateThread implements Runnable {
    ...
    public void run() {
      ...
      //queue.add(new Version(versionCodeLocal, versionCodeRemote));
        ...
        queue.add(new Version(versionCodeLocal, versionCodeRemote,
                              versionNameLocal, versionNameRemote));
        // -----------------------------------------------------------
    ...
    private String getVersionNameLocal(Context context) {
        ...
        try {
            versionNameLocal = context.getPackageManager().getPackageInfo(packageName, 0).versionName;
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }
        return versionNameLocal;
    }

    private String getVersionNameRemote() {
        String versionNameRemote = "";
        ...
        if (null != getMHashMap()) {
            versionNameRemote = getMHashMap().get("versionName");
        }
